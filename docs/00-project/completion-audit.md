...
| Radio navigation technique | Partial source gap for PIFR-070 to PIFR-075 and worked examples for PIFR-083. CASA NDB/ADF operational notes are now mapped for PIFR-076 to PIFR-083, but VOR technique and selected two-NDB plotting examples still need source inputs. | Verify a public official VOR technique source; select exact NDBs, chart panels and geometry before adding two-NDB worked examples. |
